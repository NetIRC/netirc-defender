...
- **`defender.conf`** — hub `server` / `port` / `password`, `linktype=p10`, service identity (`servername`, `sid`, …), `channel` (control channel for operator commands), `modules=…`, paths, thresholds. **`signon_fast_console`** (default on in the example): on connect, **`verbose`** + **`version`** `scan_user` run before other modules (so **`dnsbl`** does not delay “Signed on” or CTCP VERSION); on quit / join / part / kill / nick change, **`verbose`** runs before the rest so **`seen`** (disk) and other hooks do not delay “Signed off” and similar lines. Set **`signon_fast_console=0`** for strict **`modules=`** order. Optional **`control_channel_line_delay_ms`** spaces consecutive **PRIVMSG** to the control channel and **`globops`** when sends are within ~1.5s; **unset means no delay** (older trees mistakenly reused **`ipinfo_line_delay_ms`** for that — it is not used by **`ipinfo`** here). **ipinfo** reads **`ipinfo_token`**, **`ipinfo_cache_ttl_sec`**, **`ipinfo_burst_limit`**, **`ipinfo_burst_window_sec`**, **`ipinfo_http_timeout_sec`** (see `defender.conf.example`).
